...
argv=['1','test2_1_1.arp', 'test2', 'vcf','missing=0.0','maf=0.05']
...
r = re.compile("missing*") #look for "missing" in argv array
m = filter(r.match, argv)
if m: #If user did specify a missing rate:
    m=str(m)
    m=m.split("=")
    m=str(m[1])
    m=m.replace("'", '')
    ...
    for indx, kk in enumerate(inds):
        if indx >= len(inds)/2: ### We only simulate misisng rate in pop2
            for j in range(1,23):
                chr_file=open(filenameout+'_'+maf.split(".")[1]+'maf_'+m.split(".")[1]+'missing_'+'ind_'+kk+'_chr'+str(j)+'_unphased'+'.vcf', 'w')
                ...
                chr_file.write('##FORMAT=<ID=DP,Number=1,Type=Integer,Description="Read Depth">\n')
                header=('#CHROM\tPOS\tID\tREF\tALT\tQUAL\tFILTER\tINFO\tFORMAT\t')
                ...
                #print(g)
                    cov=np.random.poisson(1) #coverage for this site
    
                    if g=='0|0':
                        a1=cov     
                        a2=0
                        flip1_p=np.random.uniform(0, 1, (a1))
                        flip_a1=len(np.where(flip1_p>1-0.010)[0]) #10% chance of flip error. How many reads will flip
                        flip_a2=0
                        a1_err=a1-flip_a1
                ...
                # For GL estimation
                    ...
                    if a2_dp[inx,indx]==0 and a1_dp[inx,indx]!=0: # if all the cov goes to one allele
                            #print('caso1', a1_dp[l,indx], a2_dp[l,indx])
                        log_GL=geno_caller_3GT([[a1, 40]*a1_dp[inx,indx]], ref, alt, all_dic )
                        ...
                        norm_PL=norm_PL.replace(' ', '')
                            ...
                            #print('caso2', a1_dp[l,indx], a2_dp[l,indx])
                        log_GL=geno_caller_3GT([[a2, 40]*a2_dp[inx,indx]], ref, alt, all_dic )
                        raw_PL=[ math.log10(10**i)*-10 for i in log_GL]
                        norm_PL=[ (round(i-min(raw_PL))) for i in raw_PL]
                        GQ=int(sorted(norm_PL)[1])
                        norm_PL=str(norm_PL).replace('.0','')
                    ...
                    elif (a1_dp[inx,indx]!=0 and a2_dp[inx,indx]!=0) and (a1_dp[inx,indx]!='.' and a2_dp[inx,indx]!='.'):  #if each allele has different depth
                            #print('caso3', a1_dp[l,indx], a2_dp[l,indx])
                            #print(a1_dp[l,indx], a2_dp[l,indx])
                            
                        log_GL=geno_caller_3GT([[a1, 40]*a1_dp[inx,indx], [a2,40]*a2_dp[inx,indx] ], ref, alt, all_dic ) 
                        raw_PL=[ math.log10(10**i)*-10 for i in log_GL]
                        norm_PL=[ (round(i-min(raw_PL))) for i in raw_PL]
                        GQ=int(sorted(norm_PL)[1])
                        norm_PL=str(norm_PL).replace('.0','')
                    ...
                    if len(np.unique(chr_genos[inx,:]))==2:
                        ...
                        out= str(j)+ '\t'+pos+'\t'+'snp_'+pos+'\t'+ref+'\t'+alt+'\t.\t.\tPR\t'+'GT:GQ:DP:AD:PL'+'\t'+str(str(phase_chr[inx,indx]).split())+':'+str(GQ)+':'+str(cov_arr[inx,indx])+':'+str(a1_dp[inx,indx])+','+str(a2_dp[inx,indx])+':'+norm_PL+'\n'
                        out=out.replace('[','')
                        out=out.replace(']','')
                        out=out.replace('"','')
                        out=out.replace("'",'')
